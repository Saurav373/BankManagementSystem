...
namespace helper {
    ...
    long long int getLastAccount() {
        ...
        if (!lastAccNum) return 158694689012;
    ...
    void generateStatementTempelate(long long accountNumber) {
        ...
        string balance = helper::getValue(accountNumber, "Balance");
        ...
        file << "-------------------------------------------\n";
        file << left << setw(20) << "Transaction Type" << setw(12) << "Amount" << "Date\n";
        file << "-------------------------------------------\n";
